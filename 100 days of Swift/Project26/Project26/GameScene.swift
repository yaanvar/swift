...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func playerCollided(with node: SKNode) {
        if node.name == "vortex" {
            ...
            player.run(sequence) { [weak self] in
                self?.createPlayer(at: self?.startPosition ?? CGPoint(x: 96, y: 672))
            ...
            player.run(sequence) { [weak self] in
                let teleport = self?.teleports.randomElement()
                self?.createPlayer(at: teleport?.position ?? CGPoint(x: 96, y: 672))
    ...
    func loadLevel() {
        ...
        for (row, line) in lines.reversed().enumerated() {
            for (column, letter) in line.enumerated() {
                ...
                if letter == "x" {
                    makeBlock(at: position)
                } else if letter == "v" {
                    makeVortex(at: position)
                } else if letter == "s" {
                    makeStar(at: position)
                } else if letter == "f" {
                    makeFinish(at: position)
                } else if letter == " " {
                    // this is an empty space – do nothing!
                } else if letter == "t" {
                    makeTeleport(at: position)
                } else {
    ...
    func makeBlock(at position: CGPoint) {
    ...
    func makeVortex(at position: CGPoint) {
    ...
    func makeStar(at position: CGPoint) {
    ...
    func makeFinish(at position: CGPoint) {
    ...
    func makeTeleport(at position: CGPoint) {
        let node = SKSpriteNode(imageNamed: "vortex")
        node.name = "teleport"
        node.color = .blue
        node.position = position
