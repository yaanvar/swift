...
struct BlobView: View {
    ...
    var body: some View {
        TimelineView(.animation) { timeline in
            ...
            Canvas { context, size in
                context
                    .fill(path(in: CGRect(x: 0, y: 0, width: size.width, height: size.height),
                               x: x,
                               x2: x2),
    ...
    func path(in rect: CGRect, x: Double, x2: Double) -> Path {
        ...
        path.addCurve(to: CGPoint(x: 0.08995*width*x, y: 0.60171*height), control1: CGPoint(x: 0.35237*width*x, y: height), control2: CGPoint(x: 0.2695*width, y: 0.77304*height))
