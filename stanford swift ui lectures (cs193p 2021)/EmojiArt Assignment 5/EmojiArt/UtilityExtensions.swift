...
extension Set where Element == EmojiArtModel.Emoji {
    mutating func toggleMatching(for emoji: EmojiArtModel.Emoji) {
        if self.contains(where: { $0.id == emoji.id }) {
            self.remove(emoji)
        } else {
            self.update(with: emoji)
        }
    }
}
